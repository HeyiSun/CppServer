...
void Logger::setFormatter(const std::string& val) {
    CppServer::LogFormatter::ptr new_val(new CppServer::LogFormatter(val));
    if (new_val ->isError()) {
        ...
        if (n["appenders"].IsDefined()) {
            for (size_t x = 0; x < n["appenders"].size(); ++x) {
                ...
                if (type == "FileLogAppender") {
                    lad.type = 1;
                    if (!a["file"].IsDefined()) {
                        std::cout << "log conifg error: fileappender file is null, " << a
                                  << std::endl;
                        continue;
                    }
                    ...
                    std::cout << "log conifg error: appender type is null, " << a
                              << std::endl;
                    continue;
                }
...
CppServer::ConfigVar<std::set<LogDefine>>::ptr g_log_defines =
    CppServer::Config::Lookup("logs", std::set<LogDefine>(), "logs config");
    ...
    LogIniter() {
        g_log_defines->addListener(0xF1E231, [](const std::set<LogDefine>& old_value,
            ...
            for (auto& i : new_value) {
                ...
                CppServer::Logger::ptr logger;
                if (it == old_value.end()) {
                    //add logger
                    logger = CPPSERVER_LOG_NAME(i.name);
                } else {
                    if (!(i == *it)) {
                        // modify logger
                        logger = CPPSERVER_LOG_NAME(i.name);
            ...
            for (auto& i : old_value) {
                ...
                if (it == new_value.end()) {
                    //delete logger
                    Logger::ptr logger = CPPSERVER_LOG_NAME(i.name);
